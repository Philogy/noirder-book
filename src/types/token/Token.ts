...
import TokenContractAbiJson from '../../target/token/Token.json' assert { type: 'json' };
...
export class TokenContract extends ContractBase {
  ...
  private constructor(
    ...
    super(completeAddress, TokenContractAbi, wallet, portalContract);
  }
  ...
  public static async at(
    ...
    return Contract.at(address, TokenContract.abi, wallet) as Promise<TokenContract>;
  }
  ...
  public static deploy(pxe: PXE, admin: AztecAddressLike) {
    return new DeployMethod<TokenContract>(Point.ZERO, pxe, TokenContractAbi, Array.from(arguments).slice(1));
  }
  ...
  public static deployWithPublicKey(pxe: PXE, publicKey: PublicKey, admin: AztecAddressLike) {
    return new DeployMethod<TokenContract>(publicKey, pxe, TokenContractAbi, Array.from(arguments).slice(2));
  }
  ...
  public methods!: {
    ...
    /** compute_note_hash_and_nullifier(contract_address: field, nonce: field, preimage: array) */
    compute_note_hash_and_nullifier: ((contract_address: FieldLike, nonce: FieldLike, preimage: FieldLike[]) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;

    /** is_minter(minter: struct) */
    is_minter: ((minter: AztecAddressLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;

    /** mint_private(to: struct, amount: field) */
    mint_private: ((to: AztecAddressLike, amount: FieldLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;
    ...
    /** shield(from: struct, to: struct, amount: field, nonce: field) */
    shield: ((from: AztecAddressLike, to: AztecAddressLike, amount: FieldLike, nonce: FieldLike) => ContractFunctionInteraction) & Pick<ContractMethod, 'selector'>;
